...
Phase 1 信号族（8 类已接入）：
  ...
  8. 宏观日历 NFP/CPI/FOMC   —— Finnhub 真实日历优先；失败时 CPI/FOMC 用官方编排表兜底
...
边界（诚实说）：
  ...
  - 没有带"一致预期 vs 实际"的实时经济日历时，事件日用硬编排表；NFP 才使用规则性启发。
...
# 2026 CPI 发布时间（BLS 官方日历）。值为美国发布日；北京夏令时约当晚 20:30。
CPI_2026 = {
...
def _macro_events_for(d: date) -> list[dict]:
    """宏观事件兜底：NFP=每月第一个周五；CPI/FOMC=官方编排表。

    CPI 不再用“每月 10-15 号工作日”猜测。2026-06-15 这类日期会误报，因为
    5 月 CPI 已在 2026-06-10 发布，下一次是 2026-07-14。
    """
    events: list[dict] = []
    iso = d.isoformat()
    # NFP：每月第一个周五
