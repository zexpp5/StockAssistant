...
## 三轮 next round 全部完成（commit `4e4d38b`）

| 项目 | 状态 | 实现 |
|---|---|---|
| **N4-A** earnings 按 surprise 方向细分 | ✅ | collect_events 给 earnings 类按 surprise_pct 分四档 (✅>+10% / ↗️0~+10% / ↘️-10~0 / ❌<-10%)；实测四档 T+5 alpha 都为负 — PEAD buy-the-rumor-sell-the-news 现象 |
| **N4-B** 三市场基准 (HK ^HSI / CN 000300) | ✅ | load_benchmark_map 从 DuckDB price_daily 拉三市场基准；calc_benchmark_returns 按 market 选；三市场都用 alpha 不再退化绝对 return |
| **N4-C** dashboard catalyst validation tab | ✅ | 左侧 nav 加「📰 催化信号验证」；新 section `#catalyst-validation` 排序表（event_type / n / T+1 mean / T+5 mean+median+hit / T+20 / σ / 信号强度）；{CATALYST_VALIDATION_JSON} embed；renderCatalystValidation() 按 T+5 mean 降序，n<5 灰显 |
| **N5+** HKEX news 慈善类噪音过滤 | ✅ | _is_hkex_noise() 黑名单 12 项 (Foundation/Charity/Hosts/Award/Festival/Community/Anniversary)；保留产品/任命/MOU/指数等真信号 |
| **N3+** 客户名自动入库 (置信度阈值) | ✅ | dump_customer_candidates 末尾自动提升 freq>=3 && distinct_tickers>=2 的候选到 data/cache/customer_whitelist_auto.json；event_calendar_hk_hkex_daily._load_auto_whitelist 把它加进运行时白名单 union；当前 60 天 0 候选 (诚实结果) |

## 当前最终完成度

催化系统全部 18+5+5+1+5+5 = **39 项 全部上线** ✅。

```
主 18 项     ████████████████ 100%
收尾 5 项    ████████████████ 100%
二轮 5 项    ████████████████ 100%
HOLD 解锁 1  ████████████████ 100%（HKEX news SSR）
N3+/N4-A/N4-B/N4-C/N5+ ████████████████ 100%
                    ────────────────
                    39/39 全清
```
...
- earnings 数据 → 当 60d 内有真订单事件时，N3+ 自动入库会自动生成；A5 PDF 摘要会触发拉
- earnings 数据 → N4 catalyst alpha 长期累积 (60d 样本不够),建议每月跑一次回看 PEAD 是否成立
- catalyst validation dashboard → 加 alpha 时序曲线 (matplotlib + base64 embed) 让趋势可见
