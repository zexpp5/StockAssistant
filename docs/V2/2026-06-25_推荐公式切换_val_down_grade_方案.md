...
> ## ✅ 2026-07-06 用户拍板：AI 推荐页全面切新公式视角（展示层）
> 用户："我最多买 5~10 只，页面就多用新公式吧，老公式证明很久没用了，放到影子里。"
>
> **执行口径（展示层完整切换，生产版本号不动）**：
> - 主榜（今日 Top 表，美股）排序 / 名次 / 综合分 / 因子拆解全部切 **val_down_grade 新公式视角**：
>   `_apply_new_formula_view` 构建期把 dual_track 全池排名注入候选行（new_rank/new_score/new_factors），
>   新公式捞入、老 Top20 没有的票补合成行（标 🆕捞入）；老公式名次退悬停提示。
> - 老公式 `tech_ai_v2_usable_data_gate` 降级为**影子基线**：后台继续生成 recommendation_picks
>   （候选池底座 + 对照验证 + 富字段来源），页面不再以它为主视角。
> - **生产 strategy_version 暂不升版**：升版会清零验证样本（n≈290 断档 2-3 周）；预注册切换标准
>   （Top20 四条件，现 2/4）继续生效，达标后再升 `tech_ai_v3_us_val_down_grade` 完成数据层切换。
> - 严选 3 只 / 双轨对照 / 切换标准判定不变（本就是新公式视角）。
> - 港/A 股不动（评级因子美股专属，非美市场无新公式）。

---

> ## 🚀 2026-07-06（晚）用户二次拍板：生产数据层正式切换
> 用户："就是得统一到新公式，整个系统都得用新公式了。"（此前已拍板页面层，本次升级为全系统）
>
> **执行**：
> - `_use_us_val_down_grade()` 默认改为 **True**：美股生产从下一次 nightly run 起用 val_down_grade，
>   strategy_version 升 `tech_ai_v3_us_val_down_grade`；HK/A 仍 legacy（评级因子美股专属）。
> - **回退开关**：`US_VAL_DOWN_GRADE_ACTIVE=0` 立即回老公式（测试覆盖回退路径）。
> - **回滚线（预注册不变）**：Top20 5日Δ 连续 5 日 < 0 → 回滚。双轨面板该条已从"切换审批"改为"回滚监控"。
> - **预注册切换标准（2/4）被用户决策人显式否决式跳过**——本段即授权记录；切换标准判定继续每日运行，
>   转为事后监控证据。
> - **下游自动跟随**（无需另改）：早报⭐ / 盘前买卖信号 / AI 配仓 optimizer / 本月动作候选 /
>   AI 跟踪与验证卡（按 latest version 过滤）→ 明早 nightly+optimizer 跑完后全链一致，
>   「本月动作 vs 严选3只不一致」问题就此消除。
> - **已知代价（用户已知悉）**：验证样本按新版本重攒（老 v2 样本 n≈290 留档不删，走势线断点重启）；
>   双轨对照不受污染（同池各选，基线 prod_recheck 从全池独立复算）。
