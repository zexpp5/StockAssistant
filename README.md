...
> 产品边界先读：[docs/V2/产品基线.md](docs/V2/产品基线.md)。这里定义“我的池子”和“AI 助手”两条主线，后续模块和代码都以它为基准。
...
**DuckDB 是 single source of truth**，飞书 Bitable 只作为机器人推送通道。

```
┌─ 写入：daily_refresh 每天/每周
│   prices · picks · reviews · snapshots · watchlist  →  stock_history.duckdb
│
├─ 读取：
│   stock_dashboard.html   ← inline JSON from DuckDB （build 时注入）
│   morning_brief.py       ← 各种 *.json 中间文件 + DuckDB snapshots
│   各 jobs/*.py            ← fetch_watchlist() 默认读 DuckDB
│
└─ 编辑 watchlist（✏️ Watchlist 编辑 tab）：
    Dashboard ──fetch──> FastAPI ──upsert──> DuckDB
    
    需先启动本地 API：
    ┌────────────────────────────────────────────────┐
    │ uvicorn stock_research.api.main:app --port 8765│
    └────────────────────────────────────────────────┘
    打开 dashboard → 左侧 ⚙️ 管理 → ✏️ Watchlist 编辑
```

**飞书剩余职责**：
- 早安简报机器人推送（IM 消息）
- watchlist 表保留作为只读历史快照（暂不删，方便回滚）

应急口子：`FEISHU_WRITE_TABLES=1 python3 <script>` 可强制启用飞书表写入（调试用）。
