"""反向验证 v5：4 因子（+ 1 月反转 Jegadeesh 1990）"""
import sys, os, json
import pandas as pd
...
def main():
    ...
    print(f"\n[2/3] 4 因子（top tertile cutoff z={cutoff:.2f}）：")
    print(f"\n  {'排':<3}{'股票':<8}{'F':>3}{'动量':>8}{'反转':>8}{'分析师':>7}{'综合z':>8}{'推荐':>5}{'α%':>9}  结果")
    print(f"  {'-'*85}")
    c = w = m = a = 0
    for _, r in df.iterrows():
        ...
        print(f"  {int(r['rank']):<3}{r['ticker']:<8}{f_s:>3}{m_s:>8}{rv_s:>8}"
    ...
    print(f"  v5 学术 4 因子（+ 反转）:      召回 {rec_rate:.0f}%   准确 {prec:.0f}%")
    print(f"\n     v5: 命中 {c}  误报 {w}  漏报 {m}  正确避开 {a}")
    df.to_json("reverse_validation_v5.json", orient="records", force_ascii=False, indent=2)
