...
<section id="ipo-junior" class="max-w-7xl mx-auto px-6 py-10" style="display:none">
  ...
  <div id="ipo-sub-unlock" class="space-y-4" style="display:none">
    <div class="bg-sky-50 border border-sky-200 rounded-xl px-4 py-3 text-sm text-sky-900">
      <strong>怎么读:</strong> 「占流通市值比例」是最关键的信号 —
  ...
  <div id="ipo-sub-junior" class="space-y-4" style="display:none">
    <div class="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-sm text-emerald-900">
      ...
      本池筛选上市 <strong>6-24 个月</strong> 的标的，按四维打分（折发行价 / 时间衰减 / 首日溢价 / 较首日跌幅）排序，
      分数越高越接近"被遗忘 + 已破发"的潜在底部位置。
      <br><span class="text-rose-700">🚫 红线 gate（step 1）：触发任一红线（SPAC / 仙股 / 微盘 / 壳公司 / 低流动性 / 30 日内大额解禁 / ST）即标 <strong>"不碰"</strong>，默认隐藏；点右侧 toggle 可看为啥被打回。</span>
      <br><span class="text-rose-700">⚠️ 通过红线 ≠ 值得买；这只是"研究起点"，仍需你做基本面 + 行业 + 技术面三重判断。</span>
    </div>
    ...
    <div class="flex flex-wrap gap-2 items-center">
      ...
      <button onclick="toggleJuniorBlocked()" id="junior-toggle-blocked" title="红线触发=SPAC/仙股/微盘/壳公司/低流动性/30日内大额解禁/ST。默认隐藏不显示这些。"
              class="px-3 py-1 text-xs rounded border border-slate-300 bg-white text-slate-700 hover:border-rose-400">🚫 显示不碰 <span id="junior-blocked-count">0</span></button>
...
let _juniorShowBlocked = false;  // step 1: 默认隐藏 verdict="不碰"
...
function _renderJuniorTable_US() {
  ...
  // 默认隐藏 verdict="不碰"（step 1 红线 gate），toggle 显示
  const blockedAll = all.filter(x => x.verdict === "不碰");
  const visibleAll = _juniorShowBlocked ? all : all.filter(x => x.verdict !== "不碰");
  ...
  const bcEl = document.getElementById("junior-blocked-count");
  if (bcEl) bcEl.textContent = "(" + blockedAll.length + ")";
  const ctEl = document.getElementById("junior-count");
  if (ctEl) ctEl.textContent = `${list.length} / ${all.length} 只 · 红线 gate 标 \"不碰\" ${blockedAll.length} 只${_juniorShowBlocked ? "（已显示）" : "（已隐藏）"}`;
  ...
  const rows = list.slice(0, 200).map(x => {
    ...
    const rowClass = x.verdict === "不碰" ? "bg-rose-50/40" : "";
...
function toggleJuniorBlocked() {
  _juniorShowBlocked = !_juniorShowBlocked;
  const btn = document.getElementById("junior-toggle-blocked");
  if (btn) {
    btn.classList.toggle("bg-rose-50", _juniorShowBlocked);
    btn.classList.toggle("border-rose-400", _juniorShowBlocked);
    btn.classList.toggle("text-rose-700", _juniorShowBlocked);
  }
  _renderJuniorTable();
}

// step 1: 红线 verdict 渲染。给"不碰"行打红条 + 列出红线标签。
function _juniorVerdictCell(x) {
  ...
  return `<div class="mt-1.5 leading-snug">
    <span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-rose-600 text-white mr-1">🚫 不碰</span>${labels}
  </div>`;
}
...
function _suggestedSizeCell(item) {
  ...
  const capLine = adv.over_hard_cap
    ? `<div class="text-[10px] text-rose-600">超25%红线</div>`
